...
const ReportDetails = () => {
  ...
  const { getAssignReportDetail } = useContext(WorkerContext);
  ...
  useEffect(() => {
    const fetchReport = async () => {
      const showLoader = !report || report._id !== id;
      if (showLoader) setLoading(true);

      try {
        const data = await getAssignReportDetail(id);
  ...
  return (
    <div className="py-6">
      ...
      {loading ? (
        ...
        <div className="text-center mt-20">No report datails found</div>
      ) : (
          ...
          <div className="flex flex-col xl:flex-row gap-6 p-3 md:p-4 bg-white rounded-2xl shadow-lg overflow-hidden">
            <div>
              <img
                ...
                alt="report"
                className="w-full h-30 sm:h-64 object-cover rounded-lg"
              />
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 p-4 flex flex-col sm:flex-row items-center  gap-5 w-full mx-auto">
            <div className="flex-shrink-0">
              <img
                ...
                className="w-30 h-30 sm:w-40 sm:h-40  rounded-full object-cover border-4 border-green-100 shadow-sm"
              />
            </div>
